...
2. 赋值语句是否正确：
   ...
   char ch = i + 1;
   ```
   * 错误，因为`i + 1`为int型，无法赋给char类型。
3. 赋值语句是否正确：
   ```java
   byte b = 19;
   short s = b + 2;
   ```
   * 错误，因为`b + 2`为int型，无法赋给short类型。
4. 试写出将String转换成double类型的语句，以及将char类型转换成String的语句，举例说明。
```java
String str = "18.2";
double d1 = Double.parseDouble(str); //使用包装类

char c1 = '文';
String str2 = c1 + ""; //增加一个空串
```